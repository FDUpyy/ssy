...
class SPOT:
    ...
    def initialize(self, level=0.98, min_extrema=False, verbose=True):
        ...
        level = level - floor(level) #floor()向下取整

        n_init = self.init_data.size

        S = np.sort(self.init_data)  # we sort X to get the empirical quantile, 从小到大排序
        ...
        if verbose:
            ...
            print('Grimshaw maximum log-likelihood estimation ... ', end='') # 'end='参数设置为空格符
